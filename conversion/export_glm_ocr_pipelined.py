"""Export GLM-OCR (zai-org/GLM-OCR, 0.9B) for the Core AI pipelined engine —
the zoo's second on-device doc-OCR (after Unlimited-OCR).

Two artifacts per run (mirrors export_qwen3_vl_pipelined.py, minus deepstack):
...
* ``<name>/`` — the TEXT DECODER LanguageBundle on the pipelined-engine
  ...
  shift_start=1<<30 -> a plain GLM text decoder.
...
Run:  cd ~/code/coreai/coreai-models && .venv/bin/python \
          ../coreai-models-community/conversion/export_glm_ocr_pipelined.py \
...
from coreai_models.models.macos.glm_ocr import (
...
def export_vision(args, name: str) -> None:
    ...
    vis = GlmOcrVisionEncoder.from_hf(
...
def main() -> None:
    ...
    if not args.skip_vision:
        export_vision(args, "glm_ocr")
    if args.skip_decoder:
        return

    print(f"loading {args.hf_id} text decoder fp16 ...")
    model = GlmOcrPipelinedForCausalLM.from_hf(
        args.hf_id, target_dtype=DTYPE, grid_h=args.grid_h, grid_w=args.grid_w)
    cfg = model.config
