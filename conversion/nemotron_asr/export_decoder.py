"""Phase 3+4 — Nemotron 3.5 ASR predictor (LSTM) + joint: re-author, export, and gate
end-to-end via the host RNNT greedy loop (run in the MAIN coreai-models venv).

Same skeleton as ../parakeet/export_decoder.py, adapted to pure RNNT (durations=[]):

  predict : token[1,1] i32 + h[2,1,640] + c[2,1,640] -> dec_out[1,640] + h'[2,1,640] + c'[2,1,640]
            (embedding -> 2x manual LSTM cell -> decoder_projector; nn.LSTM avoided per the
             Kokoro lesson — single-step, so it's just two cell steps)
  joint   : dec_out[1,640] + enc_frame[1,640] -> token_logits[1,13088]
            (head(relu(enc + dec)); blank=13087, no duration slots)

Host greedy (validated == generate() in gen_oracle.py): blank advances one frame; a non-blank
token is emitted and steps the predictor, staying on the frame; max_symbols_per_step=10 per
frame forces an advance.

Gate ladder:
  1. eager re-author: host loop on golden enc_proj -> tokens must equal oracle tokens,
     per-step logits must match oracle['step_logits'].
  2. engine: same loop driving the two .aimodels -> transcript token-for-token vs golden.

Run:  coreai-models/.venv/bin/python export_decoder.py [--dtype float32] [--skip-export]
"""
...
HID, NL, VOCAB, BLANK, MAX_SYMS = 640, 2, 13088, 13087, 10
...
class Joint(nn.Module):
    def __init__(self):
        super().__init__()
        self.head = nn.Linear(HID, VOCAB, bias=True)

    def forward(self, dec_out, enc_frame):  # [1,640] each
        return self.head(torch.relu(enc_frame + dec_out))   # [1,13088]
...
# --------------------------------------------------------------------------- RNNT host loop
def rnnt_decode(predict_fn, joint_fn, enc_proj, T, collect_logits=False):
    """predict_fn(token,h,c)->(dec,h,c) ; joint_fn(dec,enc_frame)->logits. enc_proj[T,640]."""
    h = torch.zeros(NL, 1, HID)
    c = torch.zeros(NL, 1, HID)
    dec, h, c = predict_fn(torch.tensor([[BLANK]]), h, c)
    frame, syms, emitted, logits_log = 0, 0, [], []
    steps = 0
    while frame < T and steps < MAX_SYMS * T + 16:
        logits = joint_fn(dec, enc_proj[frame:frame + 1])
        token = int(logits.argmax())
        if collect_logits:
            logits_log.append(logits[0].detach().numpy())
        steps += 1
        if token == BLANK:
            frame += 1
            syms = 0
...
def main():
    ...
    with torch.no_grad():
        emitted, logs = rnnt_decode(predict, joint, enc_proj, T, collect_logits=True)
    ...
    def export(mod, example, ins, outs, name):
        prog = export_to_coreai(mod.to(dtype), example, dynamic_shapes=None,
                                ...
                                externalize_modules=[])
        prog.optimize()
        path = art / f"nemotron_asr_{name}_{args.dtype}.aimodel"
        shutil.rmtree(path, ignore_errors=True)
        meta = rt.AIModelAssetMetadata(); meta.license = "openmdw-1.1"
    ...
    j_path = export(joint, {"dec_out": z(1, HID), "enc_frame": z(1, HID)},
                    ("dec_out", "enc_frame"), ("token_logits",), "joint")
