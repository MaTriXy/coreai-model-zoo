...
Markdown / JSON. This is the piece the zoo's per-region OCRs ([Unlimited-OCR](../unlimited-ocr/README.md),
[GLM-OCR](../glm-ocr/README.md)) leave to a separate layout detector; MinerU folds it into the same weights.

MinerU2.5 is **stock Qwen2-VL** (`Qwen2VLForConditionalGeneration`): a Qwen2-VL ViT vision tower + a
Qwen2-0.5B text decoder, no custom code. This port is the shipped [Qwen3-VL](../qwen3-vl/README.md) /
[GLM-OCR](../glm-ocr/README.md) vision idiom (the `image_embeds` + rope-shift static-input hook) with a Qwen2
